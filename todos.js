...
app.post("/lists/:todoListId/complete_all",
  ...
  catchError(async (req, res) => {
    let todoListId = req.params.todoListId;
    let completedAllTodos = await res.locals.store.completeAllTodos(+todoListId);
    if (!completedAllTodos) throw new Error("Not found.");
...
app.post("/lists/:todoListId/todos",
  ...
  catchError(async (req, res) => {
    ...
    if (!errors.isEmpty()) {
      ...
      let created = store.createTodo(+todoListId, todoTitle)
  ...
  catchError(async (req, res) => {
    ...
    try {
      ...
      if (!errors.isEmpty()) {
        ...
        let updated = await store.renameTodoList(+todoListId, todoListTitle);
...
app.post("/users/signin",
  catchError (async (req, res) => {
    ...
    let authenticated = await res.locals.store.isAuthorizedUser(username, password);
    if (!authenticated) {
